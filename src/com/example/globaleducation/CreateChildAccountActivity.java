...
public class CreateChildAccountActivity extends Activity {
			...
			public void onClick(View v) {
				...
				if (accountName.length() < 6) {
					Toast.makeText(getApplicationContext(),
							R.string.invalid_username, Toast.LENGTH_LONG)
							.show();
					return;
				}
				...
				if (!password.equals(confirmPassword)) {
					Toast.makeText(getApplicationContext(),
							R.string.nonmatching_passwords, Toast.LENGTH_LONG)
							.show();
					return;
				}
				...
				try {
					...
					new ChildSignupGetTask().execute(accountName, password,
							grade, city, state, country, name);
	...
	private class CountryListGetTask extends
	...
	private class JSONCountryResponseHandler implements
	...
	private class ChildLocationGetTask extends
		...
		protected List<String> doInBackground(String... params) {
			String URL = "http://api.geonames.org/postalCodeSearchJSON?postalcode="
					+ params[0] + "&country=" + params[1] + "&username=mjsmith";
	...
	private class JSONLocationResponseHandler implements
	...
	private class ChildSignupGetTask extends AsyncTask<String, Void, Void> {
		...
		protected Void doInBackground(String... params) {
			...
			String URL = "http://cmsc436.afh.co/php/addchild.php?username="
					+ params[0] + "&password=" + params[1] + "&level="
					+ params[2] + "&city=" + city + "&state=" + state
		...
		protected void onPostExecute(Void result) {
			...
			mClient.close();
			Toast.makeText(CreateChildAccountActivity.this, "Account Created",
